...
class PlatformBinding
{
  public:
    ...
    // Present the complete status snapshot under its feature's title with Copy and OK actions, even when idle.
    virtual void ShowStatus(std::string_view title, std::string_view message) = 0;
